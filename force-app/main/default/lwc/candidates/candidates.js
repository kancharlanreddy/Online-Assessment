     ...
     //   console.log('RestCandId : ',this.RestCandId);
        ...
        @wire(getEmailCandidate, { cand: '$RestCandId'})           
            ques({data,error}){
                ...
                if(error){
                    this.showToast('ERROR','Error in Fetching Related Candidate Details: '+error, 'error');
                }                
            }
        ...
        WiredObjects_Type({ error, data }) {     
            if (data) {
               ...
               // this.showToast('Success','Assessment ID: '+this.assess, 'success');
                ...
                this.showToast('ERROR','Error in Fetching Related Assessment ID: '+error, 'error');               
            }
        ...
        @wire(getCandidate, {email: '$email'})
                   getcan({data,error}){
                    ...
                    if (!Array.isArray(data) || data.length === 0) {
                        ...
                        .then(res => {       
                           ...
                           this.el= res[0].Email__c;                            
                            this.showToast('Success','Authorized Candidate: '+this.el, 'success');       
                        })
                        .catch(error => {
                    ...
                    if (!Array.isArray(data) || data.length === 1) {
                      ...
                      //  alert('Details : '+ this.candId+' - '+this.assess+' - '+this.name ); 
                        this.showToast('Warning','You, already registered with us.', 'warning');
                    }
                }if(error){
            ...
            @wire(getCandidateResult, {cand : '$candId'})
                ...
                if (!Array.isArray(data) || data.length === 0) {
                    ...
                    this.showToast('Alert!','We did not find your result with us!', 'success');
                   // console.log('Data1 : ',JSON.stringify(data));
                    getAssessDet({assId: this.assess})
                        ...
                        .catch(error => {                
                            this.showToast('ERROR - !','We hit some Snagh : '+error.body.message, 'error');
                        });  

                } else {
                    this.showToast('Warning','We found your result also with us!', 'warning'); //+ data[0].Score__c
        ...
        handleClick(event){
            ...
            // }                                                
             if(this.email.length>3 && this.name.length>3 && this.phone.length==10 && this.assess.length>3 && this.qual.length>=2 && this.exp.length>=3){
            var field={'Candidate_Name__c':this.name,'Email__c':this.email,'Phone__c':this.phone,'Assessment__c':this.assess, 'Date__c' : this.currentDate,
            'Qualification__c':this.qual, 'Experience__c': this.exp}; 
             ...
             .catch(error=>{               
                this.showToast('ERROR',error.body.message, 'error');
                 ...
                 this.showToast('ERROR','Please fill all mandatory fields correctly.!'+this.exp, 'error');
